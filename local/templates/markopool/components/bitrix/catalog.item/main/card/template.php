...
<!-- IMPORTANT - whithout dont work - ebaniy bitrix slider -->
...
<div class="product-card">
    <div class="status-labels">
        <ul class="status-labels__list list-reset">
            <?
            if ($arParams['SHOW_DISCOUNT_PERCENT'] === 'Y') {
                ?>
                <li class="status-labels__li" <?= ($price['PERCENT'] > 0 ? '' : 'style="display: none;"') ?>>
                    <div class="status-labels__label status-labels__label--theme--blue">
                        <span class="status-labels__text"><?= -$price['PERCENT'] ?>%</span>
                    </div>
                </li>
                <?
            }
            ?>
            <? if ($item['DISPLAY_PROPERTIES']['HIT']['VALUE_XML_ID'] == 'yes'): ?>
                <li class="status-labels__li">
                    <div class="status-labels__label status-labels__label--theme--purple">
                        <span class="status-labels__text">Хит</span>
                    </div>
                </li>
            <? endif; ?>
            <? if ($item['DISPLAY_PROPERTIES']['NEW']['VALUE_XML_ID'] == 'yes'): ?>
                <li class="status-labels__li">
                    <div class="status-labels__label status-labels__label--theme--orange">
                        <span class="status-labels__text">Новинка</span>
                    </div>
                </li>
            <? endif; ?>
        </ul>
    ...
    <!-- IMAGE -->
    <a class="link-reset" href="<?= $item['DETAIL_PAGE_URL'] ?>" title="<?= $imgTitle ?>">
        <div class="product-card__block-image">
            <?
            if (!empty($item['PREVIEW_PICTURE']['SRC'])) {
                ?>
                <img class="product-card__image" src="<?= $item['PREVIEW_PICTURE']['SRC'] ?>">
                <?
            } else {
                ?>
                <img class="product-card__image" src="<?= IMG_PATTERN ?>">
                <?
            }
            ?>
        </div>
    ...
    <div class="product-card__block-content">
        <div class="product-card__block-info">
            <span class="product-card__category"><?=$item['CATALOG_ITEM_CATEGORY_TITLE']?></span>
            <a href="<?= $item['DETAIL_PAGE_URL'] ?>"
               class="product-card__name-product link-reset"><?= $productTitle ?></a>
        </div>
        <div class="product-card__block-price">

            <? if($price['PRINT_RATIO_BASE_PRICE'] > $price['PRINT_RATIO_PRICE']):?>

                <?if (!empty($price)) :?>
                    <span class="product-card__new-price" id="<?= $itemIds['PRICE'] ?>"><?= $price['PRINT_RATIO_PRICE'] ?></span>
                <?endif;?>

                <?if ($arParams['SHOW_OLD_PRICE'] === 'Y'):?>
                    <span class="product-card__old-price"
                          id="<?= $itemIds['PRICE_OLD'] ?>"
            ...
            <? else: ?>
                ...
                <?if (!empty($price)) :?>
                    <span class="product-card__price" id="<?= $itemIds['PRICE'] ?>"><?= $price['PRINT_RATIO_PRICE'] ?></span>
                <?endif;?>

            <? endif; ?>

        </div>
        <div class="product-card__block-btns" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
            <?
            if (!$haveOffers) :
                if ($actualItem['CAN_BUY']) :
                    ?>
                    <a href="javascript:void(0)" id="<?= $itemIds['BUY_LINK'] ?>"
                       class="btn btn--size--sm-first btn--theme--outline-orange">
                        <svg class="btn__icon">
                            <use xlink:href="<?= SITE_TEMPLATE_PATH ?>/assets/icons/sprite-svg.svg#basket"/>
                        </svg>
                    ...
                    <a href="javascript:void(0)"
                       class="btn btn--size--sm-first" disabled>
                        <svg class="btn__icon">
                            <use xlink:href="<?= SITE_TEMPLATE_PATH ?>/assets/icons/sprite-svg.svg#basket"/>
                        </svg>
                        <span class="btn__text">В корзину</span>
            ...
            <a href="javascript:void(0)" id="<?= $itemIds['WISHLIST_LINK'] ?>" class="link-reset">
                <svg class="product-card__icon product-card__icon--color--orange item-wishlist-svg-icon">
            ...
            <?
            if (
                $arParams['DISPLAY_COMPARE'] && (!$haveOffers || $arParams['PRODUCT_DISPLAY_MODE'] === 'Y')
            ) : ?>
            <a href="javascript:void(0)" id="<?= $itemIds['COMPARE_LINK'] ?>" class="link-reset">
                <svg class="product-card__icon product-card__icon--color--blue item-compare-svg-icon">
